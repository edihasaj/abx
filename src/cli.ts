/**
 * abx CLI — thin wrapper that talks to the persistent server
 *
 * Flow:
 *   1. Read .abx/browse.json for port + token
...
function cleanupLegacyState(): void {
  // No legacy state on Windows — /tmp and `ps` don't exist, and abx
...
async function ensureServer(flags?: GlobalFlags): Promise<ServerState> {
  ...
  if (state && await isServerHealthy(state.port)) {
    ...
    if (desiredHash && state.configHash && state.configHash !== desiredHash) {
      console.error(`[abx] existing daemon has different config (proxy/headed mismatch).`);
      console.error(`[abx] run 'browse disconnect' first to apply --proxy/--headed.`);
      process.exit(1);
    }
    ...
    if (desiredHash && !state.configHash && (flags?.proxyUrl || flags?.headed)) {
      console.error(`[abx] existing daemon was started without --proxy/--headed.`);
      console.error(`[abx] run 'browse disconnect' first to apply new flags.`);
      process.exit(1);
    }
    ...
    if (currentVersion && state.binaryVersion && currentVersion !== state.binaryVersion) {
      console.error('[abx] Binary updated, restarting server...');
  ...
  if (process.env.BROWSE_NO_AUTOSTART === '1') {
    console.error('[abx] Server not available and BROWSE_NO_AUTOSTART is set.');
    console.error('[abx] The headed browser may have been closed. Run /open-abx-browser to restart.');
    process.exit(1);
  }
  ...
  if (state && state.mode === 'headed' && isProcessAlive(state.pid)) {
    console.error(`[abx] Headed server running (PID ${state.pid}) but not responding.`);
    console.error(`[abx] Run '/open-abx-browser' to restart.`);
    process.exit(1);
  }
  ...
  if (!releaseLock) {
    ...
    console.error('[abx] Another instance is starting the server, waiting...');
    const start = Date.now();
    while (Date.now() - start < MAX_START_WAIT) {
  ...
  try {
    ...
    if (flags?.redactedProxyUrl && flags.redactedProxyUrl !== '<no proxy>') {
      console.error(`[abx] Starting server with proxy ${flags.redactedProxyUrl}${flags.headed ? ' (headed)' : ''}...`);
    } else if (flags?.headed) {
      console.error('[abx] Starting server in headed mode...');
    } else {
      console.error('[abx] Starting server...');
    }
...
async function sendCommand(state: ServerState, command: string, args: string[], retries = 0): Promise<void> {
  ...
  try {
    ...
    if (resp.status === 401) {
      ...
      console.error('[abx] Auth failed — server may have restarted. Retrying...');
    ...
    if (err.name === 'AbortError') {
      console.error('[abx] Command timed out after 30s');
      process.exit(1);
    }
    // Connection error — server may have crashed
    if (err.code === 'ECONNREFUSED' || err.code === 'ECONNRESET' || err.message?.includes('fetch failed')) {
      if (retries >= 1) throw new Error('[abx] Server crashed twice in a row — aborting');
      console.error('[abx] Server connection lost. Restarting...');
      // Kill the old server to avoid orphaned chromium processes
...
/** Check if ngrok is installed and authenticated (native config or abx env). */
function isNgrokAvailable(): boolean {
  // Check abx's own ngrok env
  const ngrokEnvPath = path.join(stateRoot(), 'ngrok.env');
...
async function handlePairAgent(state: ServerState, args: string[]): Promise<void> {
  ...
  if (!pairResp.ok) {
    ...
    console.error(`[abx] Failed to create setup key: ${err}`);
    process.exit(1);
  }
  ...
  if (pairData.tunnel_url) {
    ...
    try {
      ...
      if (cliProbe.ok) {
        ...
        console.warn(`[abx] Tunnel returned HTTP ${cliProbe.status}, attempting restart...`);
        pairData.tunnel_url = null; // fall through to restart logic
      }
    } catch {
      console.warn('[abx] Tunnel unreachable from CLI, attempting restart...');
      pairData.tunnel_url = null; // fall through to restart logic
    }
  }
  if (pairData.tunnel_url) {
    ...
    if (ngrokAvailable) {
      console.log('[abx] ngrok detected. Starting tunnel...');
      try {
        ...
        if (tunnelResp.ok && tunnelData.url) {
          console.log(`[abx] Tunnel active: ${tunnelData.url}\n`);
          serverUrl = tunnelData.url;
        } else {
          console.warn(`[abx] Tunnel failed: ${tunnelData.error || 'unknown error'}`);
          if (tunnelData.hint) console.warn(`[abx] ${tunnelData.hint}`);
          console.warn('[abx] Using localhost (same-machine only).\n');
          serverUrl = pairData.server_url;
        }
      } catch (err: any) {
        console.warn(`[abx] Tunnel failed: ${err.message}`);
        console.warn('[abx] Using localhost (same-machine only).\n');
        serverUrl = pairData.server_url;
      }
    } else {
      console.warn('[abx] No tunnel active and ngrok is not installed/configured.');
      console.warn('[abx] Instructions will use localhost (same-machine only).');
      console.warn('[abx] For remote agents: install ngrok (https://ngrok.com) and run `ngrok config add-authtoken <TOKEN>`\n');
      serverUrl = pairData.server_url;
    }
  } else {
    serverUrl = pairData.server_url;
  ...
  if (localHost) {
    try {
      ...
      let globalRoot = `.${localHost}/skills/agent-browser`;
      try {
      ...
      console.error(`[abx] Failed to write config for ${localHost}: ${err.message}`);
      process.exit(1);
    }
    return;
  }
...
async function dispatchLive(args: string[]): Promise<never> {
  const { runtime, script } = resolveLiveScript();
  const proc = Bun.spawn([runtime, script, ...args], {
    stdout: 'inherit',
    stderr: 'inherit',
    stdin: 'inherit',
  });
...
async function main() {
  ...
  try {
    ...
    if (err instanceof ProxyConfigError) {
      console.error(`[abx] error: ${err.message}`);
      console.error(`[abx] hint: ${err.hint}`);
      process.exit(1);
    }
    throw err;
  }
  ...
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    console.log(`abx — Fast headless browser for AI coding agents

Usage: abx <command> [args...]
...
Refs:           After 'snapshot', use @e1, @e2... as selectors:
    ...
    try {
      ...
      try {
        if (fs.existsSync(termAgentScript)) {
          ...
          termProc.unref();
          console.log(`[abx] Terminal agent started (PID: ${termProc.pid})`);
        }
      } catch (err: any) {
        // Non-fatal: chat still works without the terminal agent.
        console.error(`[abx] Terminal agent failed to start: ${err.message}`);
      }
    } catch (err: any) {
      console.error(`[abx] Connect failed: ${err.message}`);
      process.exit(1);
    }
    process.exit(0);
  }
  ...
  if (command === 'pair-agent') {
    ...
    if (state.mode !== 'headed' && !hasFlag(commandArgs, '--headless')) {
      console.log('[abx] Opening abx so you can see what the remote agent does...');
      ...
      if (newState && await isServerHealthy(newState.port)) {
        ...
        console.warn('[abx] Could not switch to headed mode. Continuing headless.');
      }
    }
