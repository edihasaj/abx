/**
 * abx server — persistent Chromium daemon
 *
 * Architecture:
 ...
 *
 * State:
 *   State file: <project-root>/.abx/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.abx/browse-{console,network,dialog}.log
...
/**
 * Read ngrok authtoken from env var, ~/.abx/ngrok.env, or ngrok's native
...
function resolveNgrokAuthtoken(): string | null {
  ...
  const ngrokEnvPath = path.join(home, '.abx', 'ngrok.env');
...
function generateHelpText(): string {
  ...
  const lines = ['abx — headless browser for AI agents', '', 'Commands:'];
...
async function flushBuffers() {
  ...
  try {
    ...
    console.error('[abx] Buffer flush failed:', err.message);
  } finally {
...
const idleCheckInterval = setInterval(() => {
  ...
  if (Date.now() - lastActivity > IDLE_TIMEOUT_MS) {
    console.log(`[abx] Idle for ${IDLE_TIMEOUT_MS / 1000}s, shutting down`);
    shutdown();
  }
}, 60_000);
...
// so this branch is the normal path for /open-abx-browser.
...
if (BROWSE_PARENT_PID > 0 && !IS_HEADED_WATCHDOG) {
  ...
  setInterval(() => {
    try {
      ...
      //    /pair-agent or /open-abx-browser sessions.
      ...
      if (headed || tunnelActive) {
        console.log(`[abx] Parent process ${BROWSE_PARENT_PID} exited in ${headed ? 'headed' : 'tunnel'} mode, shutting down`);
        shutdown();
      } else if (!parentGone) {
        parentGone = true;
        console.log(`[abx] Parent process ${BROWSE_PARENT_PID} exited (server stays alive, idle timeout will clean up)`);
      }
    }
  }, 15_000);
} else if (IS_HEADED_WATCHDOG) {
  console.log('[abx] Parent-process watchdog disabled (headed mode)');
} else if (BROWSE_PARENT_PID === 0) {
  console.log('[abx] Parent-process watchdog disabled (BROWSE_PARENT_PID=0)');
}
  ...
  for (const notify of inspectorSubscribers) {
    queueMicrotask(() => {
      try { notify(event); } catch (err: any) {
        console.error('[abx] Inspector event subscriber threw:', err.message);
      }
    });
  }
}
...
async function findPort(): Promise<number> {
  ...
  if (BROWSE_PORT) {
    ...
    throw new Error(`[abx] Port ${BROWSE_PORT} (from BROWSE_PORT env) is in use`);
  }
  ...
  throw new Error(`[abx] No available port after ${MAX_RETRIES} attempts in range ${MIN_PORT}-${MAX_PORT}`);
}
...
async function handleCommandInternal(
  ...
  if (tabId !== undefined && tabId !== null) {
    ...
    try { browserManager.switchTab(tabId, { bringToFront: false }); } catch (err: any) {
      console.warn('[abx] Failed to pin tab', tabId, ':', err.message);
    }
  }
  ...
  try {
      ...
      if (isScoped && DOM_CONTENT_COMMANDS.has(command)) {
        ...
        try {
          ...
          if (strippedDescs.length > 0) {
            console.warn(`[abx] Content security: ${strippedDescs.length} hidden elements flagged on ${command} for ${tokenInfo.clientId}`);
    ...
    if (savedTabId !== null) {
      try { browserManager.switchTab(savedTabId, { bringToFront: false }); } catch (restoreErr: any) {
        console.warn('[abx] Failed to restore tab after command:', restoreErr.message);
      }
    }
    ...
    if (savedTabId !== null) {
      try { browserManager.switchTab(savedTabId, { bringToFront: false }); } catch (restoreErr: any) {
        console.warn('[abx] Failed to restore tab after error:', restoreErr.message);
      }
    }
...
async function shutdown(exitCode: number = 0) {
  ...
  try {
    ...
    console.warn('[abx] Failed to kill terminal-agent:', err.message);
  }
  ...
  try { detachSession(); } catch (err: any) {
    console.warn('[abx] Failed to detach CDP session:', err.message);
  }
  ...
  const profileDir = path.join(stateRoot(), 'chromium-profile');
...
process.on('SIGTERM', () => {
  if (hasActivePicker()) {
    console.log('[abx] Received SIGTERM but cookie picker is active, ignoring to avoid stranding the picker UI');
    return;
  }
  const headed = browserManager.getConnectionMode() === 'headed';
  if (headed || tunnelActive) {
    console.log(`[abx] Received SIGTERM in ${headed ? 'headed' : 'tunnel'} mode, shutting down`);
    shutdown();
  } else {
    console.log('[abx] Received SIGTERM (ignoring — use /stop or Ctrl+C for intentional shutdown)');
  }
});
...
process.on('uncaughtException', (err) => {
  console.error('[abx] FATAL uncaught exception:', err.message);
  emergencyCleanup();
  process.exit(1);
});
process.on('unhandledRejection', (err: any) => {
  console.error('[abx] FATAL unhandled rejection:', err?.message || err);
  emergencyCleanup();
  process.exit(1);
});
...
async function start() {
  ...
  if (proxyUrl) {
    ...
    try {
      ...
      if (err instanceof ProxyConfigError) {
        console.error(`[abx] error: ${err.message} (${err.hint})`);
        process.exit(1);
      }
      throw err;
    }
    ...
    if (parsed.scheme === 'socks5' && parsed.hasAuth) {
      ...
      console.log(`[abx] Testing SOCKS5 upstream ${redactProxyUrl(proxyUrl)}...`);
      try {
        ...
        console.log(`[abx] [proxy] upstream test ok in ${test.ms}ms (${test.attempts} attempt${test.attempts === 1 ? '' : 's'})`);
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        console.error(`[abx] [proxy] FAIL upstream ${redactProxyUrl(proxyUrl)}: ${msg}`);
        process.exit(1);
      }

      proxyBridge = await startSocksBridge({ upstream: toUpstreamConfig(parsed) });
      console.log(`[abx] [proxy] bridge listening on 127.0.0.1:${proxyBridge.port}`);
      ...
      console.log(`[abx] [proxy] using ${redactProxyUrl(proxyUrl)} (pass-through to Chromium)`);
    }
  ...
  if (xvfbDecision.spawn) {
    ...
    if (displayNum == null) {
      console.error('[abx] no free X display in range :99-:120 — refusing to clobber existing X servers');
      process.exit(1);
    }
    try {
      xvfb = await spawnXvfb(displayNum);
      process.env.DISPLAY = xvfb.display;
      console.log(`[abx] [xvfb] spawned on ${xvfb.display} (pid ${xvfb.pid})`);
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      console.error(`[abx] [xvfb] FAILED: ${msg}`);
      console.error(`[abx] [xvfb] hint: ${xvfbInstallHint()}`);
      process.exit(1);
    }
    process.on('exit', () => { try { xvfb?.close(); } catch { /* shutting down */ } });
  } else if (process.env.BROWSE_HEADED === '1') {
    console.log(`[abx] [xvfb] skipped: ${xvfbDecision.reason}`);
  }
  ...
  if (!skipBrowser) {
    ...
    if (headed) {
      ...
      console.log(`[abx] Launched headed Chromium with extension`);
    } else {
  ...
  // except /connect.  Denials are logged to ~/.abx/security/attempts.jsonl.
  const makeFetchHandler = (surface: Surface) => async (req: Request): Promise<Response> => {
    ...
    if (url.pathname === '/connect' && req.method === 'GET') {
      ...
      // Welcome page — served when abx launches in headed mode
      if (url.pathname === '/welcome') {
        const welcomePath = (() => {
          // Gate ABX_SLUG on a strict regex BEFORE interpolating it into
          // the filesystem path. Without this, a slug like "../../etc/passwd"
          // would resolve to ~/.abx/projects/../../etc/passwd/... — path
          ...
          const rawSlug = process.env.ABX_SLUG || 'unknown';
          const slug = /^[a-z0-9_-]+$/.test(rawSlug) ? rawSlug : 'unknown';
          const homeDir = process.env.HOME || process.env.USERPROFILE || '/tmp';
          const projectWelcome = `${homeDir}/.abx/projects/${slug}/designs/welcome-page-20260331/finalized.html`;
          if (fs.existsSync(projectWelcome)) return projectWelcome;
          // Fallback: built-in welcome page from abx install.  Reject
          // SKILL_ROOT values containing '..' for the same defense-in-depth
          // reason as the ABX_SLUG regex above.  Not exploitable today
          // (env set at install time), but the gate is one check.
          const rawSkillRoot = process.env.ABX_SKILL_ROOT || `${homeDir}/.claude/skills/agent-browser`;
        ...
        if (welcomePath) {
          try {
            ...
            console.error('[abx] Failed to read welcome page:', welcomePath, err.message);
          }
        }
        // No welcome page found — serve a simple fallback (avoid ERR_UNSAFE_REDIRECT on Windows)
        return new Response(
          `<!DOCTYPE html><html><head><title>abx</title>
          ...
          <body><div class="msg"><div class="gold">◈</div><p>abx ready.</p><p style="font-size:.85em">Waiting for commands from Claude Code.</p></div></body></html>`,
      ...
      if (url.pathname === '/connect' && req.method === 'POST') {
        ...
        try {
          ...
          console.log(`[abx] Remote agent connected: ${session.clientId} (scopes: ${session.scopes.join(',')})`);
          return new Response(JSON.stringify({
            token: session.token,
      ...
      if (url.pathname.startsWith('/token/') && req.method === 'DELETE') {
        ...
        console.log(`[abx] Revoked token for: ${clientId}`);
          ...
          if (tunnelActive && tunnelUrl) {
            try {
              ...
              if (probe.ok) {
                ...
                console.warn(`[abx] Tunnel probe failed (HTTP ${probe.status}), marking tunnel as dead`);
                await closeTunnel();
              }
            } catch {
              console.warn('[abx] Tunnel probe timed out or unreachable, marking tunnel as dead');
              await closeTunnel();
            }
          }
          return new Response(JSON.stringify({
      ...
      if (url.pathname === '/tunnel/start' && req.method === 'POST') {
        ...
        if (tunnelActive && tunnelUrl && tunnelServer) {
          ...
          console.warn('[abx] Cached tunnel is dead, restarting...');
          await closeTunnel();
        }

        // 1) Resolve ngrok authtoken from env / .abx / native config
        ...
        try {
          ...
          tunnelActive = true;
          console.log(`[abx] Tunnel listener bound on 127.0.0.1:${tunnelPort}, ngrok → ${tunnelUrl}`);
          ...
          start(controller) {
            ...
            const unsubscribe = subscribe((entry) => {
              try {
                ...
                console.debug('[abx] Activity SSE stream error, unsubscribing:', err.message);
                unsubscribe();
              }
            });
            ...
            const heartbeat = setInterval(() => {
              try {
                ...
                console.debug('[abx] Activity SSE heartbeat failed:', err.message);
          ...
          start(controller) {
            ...
            const notify: InspectorSubscriber = (event) => {
              try {
                ...
                console.debug('[abx] Inspector SSE stream error:', err.message);
            ...
            const heartbeat = setInterval(() => {
              try {
                ...
                console.debug('[abx] Inspector SSE heartbeat failed:', err.message);
                clearInterval(heartbeat);
                inspectorSubscribers.delete(notify);
              }
            }, 15000);
    ...
    try {
      ...
      if (currentUrl === 'about:blank' || currentUrl === '') {
        ...
        page.goto(`http://127.0.0.1:${port}/welcome`, { timeout: 3000 }).catch((err: any) => {
          console.warn('[abx] Failed to navigate to welcome page:', err.message);
        });
      }
    } catch (err: any) {
      console.warn('[abx] Welcome page navigation setup failed:', err.message);
    }
  }
    ...
    if (fs.existsSync(stateDir)) {
      ...
      for (const file of fs.readdirSync(stateDir)) {
        ...
        if (Date.now() - stat.mtimeMs > SEVEN_DAYS) {
          ...
          console.log(`[abx] Deleted stale state file: ${file}`);
        }
      }
    }
  } catch (err: any) {
    console.warn('[abx] Failed to clean stale state files:', err.message);
  }

  console.log(`[abx] Server running on http://127.0.0.1:${port} (PID: ${process.pid})`);
  console.log(`[abx] State file: ${config.stateFile}`);
  console.log(`[abx] Idle timeout: ${IDLE_TIMEOUT_MS / 1000}s`);
  ...
  if (process.env.BROWSE_TUNNEL === '1') {
    const authtoken = resolveNgrokAuthtoken();
    if (!authtoken) {
      console.error('[abx] BROWSE_TUNNEL=1 but no NGROK_AUTHTOKEN found. Set it via env var or ~/.abx/ngrok.env');
    } else {
      ...
      try {
        ...
        console.error(`[abx] Failed to start tunnel: ${err.message}`);
    ...
    try {
      ...
      tunnelActive = true;
      const tunnelPort = boundTunnel.port;
      console.log(`[abx] Tunnel listener bound (local-only test mode) on 127.0.0.1:${tunnelPort}`);
      const stateContent = JSON.parse(fs.readFileSync(config.stateFile, 'utf-8'));
      ...
      console.error(`[abx] BROWSE_TUNNEL_LOCAL_ONLY=1 listener bind failed: ${err.message}`);
    }
  }
}

start().catch((err) => {
  console.error(`[abx] Failed to start: ${err.message}`);
